...
<head>
	...
	<?php
		...
		echo $this->Html->css('prelaunch.cache', null, array('inline' => true));
...
<?php
	...
	if (!empty($background_attachment['Attachment']['id']) && empty($this->request->params['requested']) && $this->request->params['controller'] != 'images'):
		$original_image = $this->Html->url($this->Html->getImageUrl('PageLogo', $background_attachment['Attachment'], array('dimension' => 'original')));
		$big_thumb_image = $this->Html->url($this->Html->getImageUrl('PageLogo', $background_attachment['Attachment'], array('dimension' => 'big_thumb')));
		$prelaunch_stretch_type = Configure::read('prelaunch.stretch_type');
	endif; 		
?>
<body>
<div  class="content">
	<?php if(!empty($original_image)):?>
	<div id="<?php echo $prelaunch_stretch_type;?>" style="backround:url('<?php echo $original_image; ?>') repeat left top;">
