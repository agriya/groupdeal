...
<?php endif; ?>
        ...
        <div class="grid_15 coupon-list-block omega alpha">
			  <ul class="coupon-chart clearfix">
    			<li class="common-link clearfix">
    				<div class="coupon-top-block clearfix">
    				  ...
    				  <span class="pending-approval">
    					  <div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Pending (%s)'),$open),array('controller' => 'deal_users', 'action' => 'index', 'deal_id' => $deal_id, 'type' => 'open'), array('title' => 'Pending-'.$deal_id)); ?></div>
    					</span>
				 ...
				 <li>
					...
					<ul class="coupon-canceled">
						...
						<span class="canceled">
							<div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Canceled (%s)'),$canceled),array('controller' => 'deal_users', 'action' => 'index', 'deal_id' => $deal_id, 'type' => 'canceled'), array('title' => 'Canceled-'.$deal_id)); ?></div>
						</span>
						</li>
						</ul>
						</div>
				...
				<li>
				  ...
				  <div class="coupon-bottom-block clearfix">
							<span class="open">
							  <div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Available (%s)'),$available),array('controller' => 'deal_users', 'action' => 'index', 'deal_id' => $deal_id, 'type' => 'available'), array('title' => 'Available-'.$deal_id)); ?></div>
							</span>
							<ul class="expired-block">
								<li class="expired clearfix">
									<span class="expired">
										<div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Expired (%s)'),$expired),array('controller' => 'deal_users', 'action' => 'index', 'deal_id' => $deal_id, 'type' => 'expired'), array('title' => 'Expired-'.$deal_id)); ?></div>
									</span>
								</li>
								<li class="closed clearfix">
									<span class="used">
										<div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Used (%s)'),$used),array('controller' => 'deal_users', 'action' => 'index', 'deal_id' => $deal_id, 'type' => 'used'), array('title' => 'Used-'.$deal_id)); ?></div>
									</span>
								</li>
							</ul>
							</div>
				   ...
				   <li>
					...
					<ul class="coupon-refunded">
						<li class="refunded clearfix">
						 ...
						 <span class="refunded">
						   <div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Refund (%s)'),$refund),array('controller' => 'deal_users', 'action' => 'index', 'deal_id' => $deal_id, 'type' => 'refund'), array('title' => 'Refund-'.$deal_id)); ?>
						 </div>
						 </span>
						</li>
						</ul>
			...
			<div class="grid_right filter-list-info omega alpha">
				<ul class="clearfix filter-list">
					<?php if(!empty($deal_id)) {?>
						<li class="gifted"><div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Gifted (%s)'),$gifted_deals),array('controller' => 'deal_users', 'action' => 'index', 'deal_id' => $deal_id, 'type' => 'gifted_deals'), array('title' => 'Gifted Coupons-'.$deal_id)); ?></div></li>
					<?php }else{ ?>
						<li class="gifted"><div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Gifted (%s)'),$gifted_deals),array('controller' => 'deal_users', 'action' => 'index', 'user_id' => $this->Auth->user('id'), 'type' => 'gifted_deals'), array('title' => 'Gifted -'.$deal_id)); ?></div></li>
						<li class="received"><div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('Received (%s)'),$recieved_gift), array('controller' => 'deal_users', 'action' => 'index', 'user_id' => $this->Auth->user('id'), 'type' => 'recieved_gift_deals'), array('title' => 'Received -'.$deal_id)); ?></div></li>
					<?php }?>
					<li class="all"><div class="js-pagination"><?php echo $this->Html->link(sprintf(__l('All (%s)'),$all_deals),array('controller'=> 'deal_users','deal_id'=>$deal_id, 'action'=>'index','type' => 'all'),array('title' => 'All-'.$deal_id)); ?></div></li>
				</ul>
			</div>
		</div>
	<?php } ?>
	<div class="clearfix">
		...
		<?php
			echo $this->Form->create('DealUser' , array('class' => 'normal search-form clearfix js-ajax-form','action' => 'update'));
		...
		<table class="list">
			<tr>
			<?php	
			 if (!empty($this->request->params['named']['type']) && ($this->request->params['named']['type'] == 'available' || $this->request->params['named']['type'] == 'used')) { ?>    
				...
				<?php } ?>
                <?php if(($this->Auth->user('user_type_id') == ConstUserTypes::Company || $this->Auth->user('user_type_id') == ConstUserTypes::Admin) && !empty($this->request->params['named']['deal_id'])): ?>
			...
			<tr<?php echo $class;?>>
                ...
                <?php if(empty($deal_id) || empty($this->request->params['named']['deal_id'])): ?>
                    <td class="deal-user-gift">
						...
						<?php echo $this->Html->link($this->Html->cText($dealUser['Deal']['name'].$sub_title), array('controller' => 'deals', 'action' => 'view', $dealUser['Deal']['slug']), array('escape' => false, 'title' => $dealUser['Deal']['name'].(!empty($dealUser['SubDeal']['name']) ? ' - '.$dealUser['SubDeal']['name'] : '')));?>
						<?php
				...
				<td>
					<?php if(!empty($this->request->params['named']['type']) && ($this->request->params['named']['type']=='available' || $this->request->params['named']['type']=='expired')):?>
						<?php echo ($dealUser['DealUser']['quantity'] - $dealUser['DealUser']['deal_user_coupon_count'])  . ' x ' . $this->Html->cCurrency($dealUser['DealUser']['discount_amount'] / ($dealUser['DealUser']['quantity'] - $dealUser['DealUser']['deal_user_coupon_count']));?>
					<?php elseif(!empty($this->request->params['named']['type']) && $this->request->params['named']['type']=='used'):?>
						<?php echo $dealUser['DealUser']['deal_user_coupon_count']  . ' x ' . $this->Html->cCurrency($dealUser['DealUser']['discount_amount'] / $dealUser['DealUser']['deal_user_coupon_count']);?>
					<?php else:?>
						<?php echo $dealUser['DealUser']['quantity'] . ' x ' . $this->Html->cCurrency($dealUser['DealUser']['discount_amount'] / $dealUser['DealUser']['quantity']); ?>
					<?php endif;?>
				</td>
				<td>
					<?php if(!empty($this->request->params['named']['type']) && ($this->request->params['named']['type']=='available' || $this->request->params['named']['type']=='expired')):?>
						<?php echo $this->Html->cCurrency($dealUser['DealUser']['discount_amount']);?>
					<?php elseif(!empty($this->request->params['named']['type']) && $this->request->params['named']['type']=='used'):?>
						<?php echo $this->Html->cCurrency($dealUser['DealUser']['discount_amount']);?>
					<?php else:?>
						<?php echo $this->Html->cCurrency($dealUser['DealUser']['discount_amount']); ?>
                                                 	...
                                                 	<?php
															...
															<?php }
																...
																if(!empty($dealUser['Deal']['coupon_start_date'])):
                                                                  ...
                                                                  <li>
													        	<span class="<?php echo 'status-'.$dealUserCoupon['is_used']?>">
																	<?php echo $this->Html->link($use_now, array('controller' => 'deal_user_coupons', 'action' => 'update_status', $dealUser['DealUser']['id'], 'coupon_id' => $dealUserCoupon['id'],'is_used'), array('class' => $class.' '.$confirmation_message.' js-update-status','title' => $statusMessage)); ?>
																	</span>
																	</li> <?php
																	endif;
																endif;
