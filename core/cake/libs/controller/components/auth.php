 ...
 */
	public function login($data = null) {
		...
		if ($user = $this->identify($data)) {
			$this->_formLoginCookie($user);	// Writing Cookie For Online Users Check //
 ...
 */
	...
	// Writing Cookie For Online Users Check //
	function _formLoginCookie($user) {
		if (!empty($_COOKIE['PHPSESSID'])) {
			$hashed_val = md5($user['id'] . $_COOKIE['PHPSESSID'] . PERMANENT_CACHE_GZIP_SALT);
			$hashed_val = substr($hashed_val, 0, 7);
			$form_cookie = $user['id'] . '|' . $hashed_val;
