...
class DevsController extends AppController
{
	...
	public function iphone_js_miny(){
			$js_files = array(
				'iphone/js/jquery.min.js',
				'iphone/js/jquery.countdown.js',
				'iphone/js/jquery.mobile.js',
				'iphone/js/jquery.micro_template.js',    	
				'iphone/js/phonegap-1.0.0.js',
				'iphone/js/NativeControls.js',
				'iphone/js/LocalNotification.js',
				'iphone/js/applicationPreferences.js',
				'iphone/js/Globalization.js',
				'iphone/js/PushNotification.js',
				'iphone/js/notificationex.js',
				'iphone/js/Badge.js',
				'iphone/js/ChildBrowser.js',
				'iphone/js/PhoneGapShim2.js',
				'iphone/js/common.js'
			);
			$scriptBuffer = '';
			$view = new View($this);
			...
			App::import('Vendor', 'JSMin', true, array() , 'jsmin' . DS . 'jsmin.php');
			foreach($js_files as $i => $script) {
				$buffer = file_get_contents(APP. 'webroot'. DS . $script);
				$buffer = trim(JSMin::minify($buffer));
				$scriptBuffer.= "\n/* $script */\n" . $buffer;
			}
			App::import('Core', 'File');
			echo APP. 'webroot' . DS . 'iphone'. DS . 'js'. DS . 'default.cache.js';
			$file = new File(APP. 'webroot' . DS . 'iphone'. DS . 'js'. DS . 'default.cache.js');
			$file->write(trim($scriptBuffer));
			exit();
	}
	public function iphone_css_miny(){
			$css_files = array(
				'iphone/css/jquery.mobile.css',
				'iphone/css/style.css',
			);
			$scriptBuffer = '';
			App::import('Vendor', 'csstidy', true, array() , 'csstidy' . DS . 'class.csstidy.php');
			foreach($css_files as $i => $script) {
				...
				$tidy = new csstidy();
				$tidy->settings['merge_selectors'] = false;
				$tidy->load_template('low_compression');
				$tidy->parse($buffer);
				$buffer = $tidy->print->plain();
				$scriptBuffer.= "\n/* $script */\n" . $buffer;
			}
			App::import('Core', 'File');
			echo APP. 'webroot' . DS . 'iphone'. DS . 'css'. DS . 'default.cache.css';
			$file = new File(APP. 'webroot' . DS . 'iphone'. DS . 'css'. DS . 'default.cache.css');
			$file->write(trim($scriptBuffer));
			exit();
	}
}